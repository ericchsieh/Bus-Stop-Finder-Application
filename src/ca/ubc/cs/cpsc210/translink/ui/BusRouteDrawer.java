    ...
    public void plotRoutes(int zoomLevel) {
        ...
        if (stop != null) {
            ...
            for (Route r : stop.getRoutes()) {
                ...
                for (RoutePattern pat : r.getPatterns()) {

                    LatLon src = pat.getPath().get(0);
                    LatLon dst = pat.getPath().get(1);
                    ArrayList<GeoPoint> wayPoints = new ArrayList<>();
                    Polyline polyline = new Polyline(context);

                    for (LatLon latLon : pat.getPath()) {
                        if (Geometry.rectangleIntersectsLine(northWest, southEast, src, dst)) {
                            src = dst;
                            dst = latLon;

                            GeoPoint newGeoPoint = new GeoPoint(latLon.getLatitude(), latLon.getLongitude());
                            wayPoints.add(newGeoPoint);} else {
                            src = dst;
                            dst = latLon;
                        }
